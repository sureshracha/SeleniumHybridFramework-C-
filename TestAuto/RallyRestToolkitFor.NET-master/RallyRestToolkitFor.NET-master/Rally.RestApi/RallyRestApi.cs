	...
	public class RallyRestApi
	{
		...
		private DynamicJsonObject DoGet(Uri uri, bool retry = true, int retryCounter = 1)
		{
			...
			try
			{
                ...
                //    {
                //        throw new Exception(string.Format("Server Error( HTTP {0}:{1}).", rsp.StatusCode, rsp.StatusDescription));
                //    }
                //    DataContractJsonSerializer jsonSerializer = new DataContractJsonSerializer(typeof(HttpWebResponse));
                //    object objResponse = jsonSerializer.ReadObject(rsp.GetResponseStream());
                //    HttpWebResponse jsonRespon = objResponse as HttpWebResponse;
                //   // return jsonRespon;
                //}

                //HttpWebRequest httpwebrequest = (HttpWebRequest)WebRequest.Create(uri);
                //httpwebrequest.Method = WebRequestMethods.Http.Get;
                //httpwebrequest.Accept = "application/json;";//"text/json";
